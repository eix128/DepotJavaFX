...
public class CtzJniUtils {
     ...
     * @throws ru.concerteza.util.jni.CtzJniException
     */
    public static void loadJniLib(String name, File dirPath) {
        final String filename;
        switch (CURRENT_PLATFORM) {
            case UNKNOWN:
                throw new ru.concerteza.util.jni.CtzJniException("Cannot determine platform, os.name: [" + getProperty("os.name") + "]," +
        ...
        if (!(target.exists() && target.isFile())) throw new ru.concerteza.util.jni.CtzJniException("File not found: [" + target.getPath() + "]");
        try {
            System.load(target.getPath());
        } catch (Exception e) {
            throw new ru.concerteza.util.jni.CtzJniException(e);
        } catch (Error e) {
            throw new ru.concerteza.util.jni.CtzJniException(e);
        }
    }
     ...
     * Loads JNI library with proper platform postfix from {@code <provided_class_jar>/lib/native} directory
     *
     * @param mainPath class to build path from
     * @param fileName list of libraries names
     */
    public static void loadJniLibsFromStandardPath(String mainPath , String... fileName) {
        File jarPath = new File(mainPath);
        String postfix = "lib" + File.separator + "natives";
        File nativeLibsPath = new File(jarPath, postfix).getAbsoluteFile();
        for(String na : fileName) loadJniLib(na, nativeLibsPath);
    }
}