 ...
 */
public class DynamicFilterJPA implements ObservableList {
    ...
    public DynamicFilterJPA(LangUtils langUtils, EntityManager entityManager) {
    ...
    public void filter(DepotsEntity depotsEntity, ProductsEntity productsEntity, CustomerEntity customerEntity, ProcessType processType) {
...
//        Predicate<String> p = cb.like(postgresqlCastFunction, "10%");
        ...
        if (processType != null && processType != ProcessType.ALL) {
...
//        final Predicate and2 = criteriaBuilder.and(and1, equal3);
        ...
        final List<ProcessItem> collect = resultList.parallelStream().map(new Function<ProcessEntity, ProcessItem>() {
            ...
            public ProcessItem apply(ProcessEntity processEntity) {
                final ProcessItem from = ProcessItem.from(langUtils,processEntity);
                return from;
            }
